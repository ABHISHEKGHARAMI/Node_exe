...
request({url : curl},(error,response)=>{
    ...
    const lat = data1.features[0].center[0];
    const long = data1.features[0].center[1]
    console.log(long);
});
const url = "http://api.weatherstack.com/current?access_key=1d2c0ef28050254b8192ef89de2a69d0&query=22.9295,88.424";
request({url: url},(error,response)=>{
    /*
    const data = JSON.parse(response.body);
    console.log('tempareture is :',data.current.temperature,' but feels like :',data.current.feelslike);
